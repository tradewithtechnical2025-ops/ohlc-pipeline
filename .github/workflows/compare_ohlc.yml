...
Usage:
  python compare_ohlc.py                  # all NSE symbols from classification.json
  python compare_ohlc.py --days 30        # last N trading days window (default: 30)
  python compare_ohlc.py --limit 50       # only first N symbols (for testing)
  python compare_ohlc.py --sym RELIANCE TCS INFY   # specific symbols only

Env vars required:
  UPSTOX_TOKEN   — Upstox Bearer token
  WORKER_URL     — Cloudflare Worker base URL  (e.g. https://xxx.workers.dev)
  WORKER_TOKEN   — Worker secret token

Output:
  ohlc_comparison.csv   — full row-level diff
...
UPSTOX_INSTRUMENTS = "https://assets.upstox.com/market-quote/instruments/exchange/NSE.csv.gz"

R2_CHUNKS      = 8
WORKER_HEADERS = {"X-Secret-Token": WORKER_TOKEN}

CONCURRENCY  = 5      # Upstox concurrent requests
RATE_DELAY   = 0.4    # seconds between requests per slot
RETRY        = 3

MISMATCH_PCT = 0.05   # flag if close diff > 0.05%
...
async def load_classification(client) -> list:
    data = await r2_download(client, "classification.json")
    if not data or not isinstance(data, list):
        print("[ERROR] classification.json missing"); sys.exit(1)
    return data

async def load_r2_ohlc(client) -> dict:
    ...
    print(f"  [R2] {len(all_data)} stocks loaded from {R2_CHUNKS} chunks")
    return all_data

# ── Upstox instruments → ISIN map ────────────────────────────────────────────

async def build_isin_map(client) -> dict:
    print("  Downloading NSE.csv.gz from Upstox…")
    try:
        r = await client.get(UPSTOX_INSTRUMENTS, timeout=60)
        r.raise_for_status()
        raw = gzip.decompress(r.content).decode("utf-8")
    except Exception as e:
        print(f"  [ERROR] Instruments download failed: {e}"); return {}

    isin_map = {}
    reader = csv.DictReader(io.StringIO(raw))
    for row in reader:
        seg   = row.get("exch_seg", "").strip()
        itype = row.get("instrumenttype", "").strip().upper()
        sym   = row.get("tradingsymbol", "").strip().upper()
        isin  = row.get("isin", "").strip()
        if seg == "NSE" and itype == "EQ" and sym and isin:
            isin_map[sym] = isin

    print(f"  ISIN map: {len(isin_map)} NSE EQ symbols")
    return isin_map

# ── Upstox OHLC fetch ─────────────────────────────────────────────────────────

async def fetch_upstox(client, sem, symbol, isin, from_date, to_date) -> tuple:
    ...
    for attempt in range(RETRY):
        ...
        for row in candles:
            ...
            if from_date <= d <= to_date:
                result[d] = {"o": row[1], "h": row[2], "l": row[3], "c": row[4], "v": row[5]}
...
def r2_to_date_map(stock, from_date, to_date) -> dict:
    result = {}
    for i, d in enumerate(stock.get("d", [])):
        if from_date <= d <= to_date:
            result[d] = {
                "o": stock["o"][i], "h": stock["h"][i],
                "l": stock["l"][i], "c": stock["c"][i],
                "v": (stock.get("v") or [0]*len(stock["d"]))[i],
            }
    return result

# ── Compare ───────────────────────────────────────────────────────────────────

def compare(symbol, up_data, r2_data) -> list:
    rows = []
...
def make_summary(symbol, rows, up_data, r2_data) -> dict:
    total = len(rows)
    mm    = sum(1 for r in rows if r["mismatch"])
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Step 1 — symbol list from classification
        ...
        # Step 2 — ISIN map
        print("\n[2/4] ISIN map from Upstox instruments…")
        isin_map  = await build_isin_map(client)
        fetchable = [s for s in nse_symbols if s in isin_map]
        no_isin   = [s for s in nse_symbols if s not in isin_map]
        print(f"  Fetchable : {len(fetchable)}  |  No ISIN: {len(no_isin)}")
        if no_isin[:5]: print(f"  No ISIN sample: {no_isin[:5]}")

        # Step 3 — R2 OHLC
        print("\n[3/4] Loading R2 OHLC chunks…")
        r2_all = await load_r2_ohlc(client)

        # Step 4 — Upstox fetch + compare
        ...
        for batch_start in range(0, len(fetchable), BATCH):
            ...
            tasks = [
                fetch_upstox(client, sem, sym, isin_map[sym], from_date, to_date)
                for sym in batch
            ]
